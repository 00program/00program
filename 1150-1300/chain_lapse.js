        ...
        if (window.__TM) {
            ...
            if (!ok) {
                window.__TM.errors.push({
                    ...
                    file: 'chain_lapse.js', line: 0, col: 0, stack: ''
                });
            }
            if (window.__TM.render) window.__TM.render();
        }
...
(async function () {
    ...
    try {
        ...
        const ITERS = params.has("iters") ? parseInt(params.get("iters"), 10) : 600;
        const SPRAY_NUM = params.has("spray") ? parseInt(params.get("spray"), 10) : 0x300;
        ...
        tmDiag('iters', ITERS);
        tmDiag('spray_num', SPRAY_NUM);
        tmDiag('stop_precommit', STOP_PRECOMMIT ? 1 : 0);
        ...
        const PRE_SUSPEND_MS = params.has("presleep")
            ? parseInt(params.get("presleep"), 10) : 20;
        ...
        const ATTEMPTS = params.has("attempts")
            ? parseInt(params.get("attempts"), 10) : 30;
        ...
        tmDiag('wide_window', wideWindow ? 1 : 0);
        tmDiag('wide_len', wideLen);
        tmDiag('pre_suspend_ms', PRE_SUSPEND_MS);
        tmDiag('yield_cap', YIELD_CAP);
        tmDiag('probe_cap', PROBE_CAP);
        tmDiag('strict_tcp', STRICT_TCP ? 1 : 0);
        tmDiag('attempts_max', ATTEMPTS);
        tmDiag('misfires_max', MAX_MISFIRES);
        ...
        tmDiag('one_core', ONE_CORE);
        tmDiag('avail_cores', availCores.join(','));
        ...
        // --- Diagnóstico del race ---
        tmDiag('race_attempts_used', attemptsUsed);
        tmDiag('race_detector_fired', detectorFired);
        tmDiag('race_real_frees', realFrees);
        tmDiag('race_benign_hits', benignHits);
        tmDiag('race_reclaim_failed', reclaimFailed ? 1 : 0);
        tmDiag('race_misfire_cap', misfireCap ? 1 : 0);
        tmDiag('race_never_started', neverStarted);
        tmDiag('race_suspend_fail', suspendFail);
        tmDiag('race_yields_total', yieldTotal);
        tmDiag('race_rendezvous', rendezvous);
        tmDiag('race_probe_total', probeTotal);
        tmDiag('race_probe_max', probeMax);
        tmDiag('race_resuspend_fail', resuspendFail);
        tmDiag('race_in_window_seen', inWindowSeen);
        tmDiag('race_too_late_seen', tooLateSeen);
        tmDiag('race_dup_seen', dupSeen);
        tmDiag('race_last_poll_err', hx(lastPollErr));
        tmDiag('race_last_tcp_state', lastTcp);
        tmDiag('race_err0', hx(raceErr0));
        tmDiag('race_err1', hx(raceErr1));
        tmDiag('race_setup_fail', setupFail || 'none');
        tmDiag('race_ms', raceMs);
        tmDiag('race_stuck_ok', stuckOk);
        tmDiag('race_stuck_fail', stuckFail);
        tmDiag('race_stuck_bytes', stuckBytes);
        ...
        if (twins) {
            ...
            const leakOk = (function () {
                ...
                tmDiag('leak_reqs2_base', reqs2Base);
                tmDiag('leak_reqs1_aligned', reqs1Aligned.toString());
                tmDiag('leak_aio_info', aioInfoAddr.toString());
                tmDiag('leak_ar2_file', leakFp.toString());
                ...
                tmDiag('leak_evf_cv', evfCv.toString());
                tmDiag('leak_reqs2', reqs2Addr.toString());
                tmDiag('leak_target_id', hx(targetId));
                tmDiag('leak_evf', hx(evf));
                ...
                tmDiag('kread_calls', kreadCalls);
                tmDiag('kread_fail', kreadFail);
                tmDiag('short_reads', shortReads);
                tmDiag('have_curproc', haveCurproc ? 1 : 0);
                tmDiag('curproc_from', curprocFrom);
                ...
                tmDiag('kstr_residue', '0x' + KSTR_RESIDUE.toString(16));
                ...
                if (kstrOff >= 0) {
                    ...
                    if (hdrOk) {
                        ...
                        tmDiag('kbase', kbase.toString());
                        tmDiag('kstr_off', '0x' + kstrOff.toString(16));
                    }
                } else {
                ...
                // Diagnóstico final del bloque stage5
                tmDiag('kv_up', kv ? 1 : 0);
                tmDiag('kbase', kbase ? kbase.toString() : 'null');
                tmDiag('ipv6_open', ipv6Socks.length);
                tmDiag('pktopts_twins', pktoptsTwins.join(','));
                tmDiag('twin_socks', twinSocks.join(','));
                tmDiag('repaired', repaired ? 1 : 0);
                tmDiag('jailbroken', jailbroken ? 1 : 0);
                tmDiag('kpatched', kpatched ? 1 : 0);
                tmDiag('payload_running', payloadRunning ? 1 : 0);
                tmDiag('cleanup_done', cleanupDone ? 1 : 0);
        ...
        tmDiag('final_reboot_required', rebootRequired ? 1 : 0);
        tmDiag('final_committed', committed ? 1 : 0);
        tmDiag('final_committed2', committed2 ? 1 : 0);
        tmDiag('final_repaired', repaired ? 1 : 0);
        tmDiag('final_cleanup_done', cleanupDone ? 1 : 0);
        tmDiag('final_jailbroken', jailbroken ? 1 : 0);
        tmDiag('final_kpatched', kpatched ? 1 : 0);
        tmDiag('final_payload_running', payloadRunning ? 1 : 0);
